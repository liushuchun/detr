...
IMAGE_ROOT = "/data/workspace/shuchun/detr/datas/images"

tradition_to_simple = dict()
for t, s in zip(TRADITION, SIMPLE):
    tradition_to_simple[t] = s


def convert_front_to_coco(front_path):
    key_map = dict()
    with open("tools/dict_keys.txt") as f:
    ...
    infos = dict(description="text char dataset", url="http://cocodataset.org", version="1.0",
                 year=2020, contributor="liushuchun", data_created="2020/11/30")
    images = []
    annotations = []
    categories = []

    for key, val in key_map.items():
        category = dict(supercategory="text", id=key, name=val)
    ...
    for data in datas:
        ...
        for character in characters:
            ...
            for ch, pt in zip(transcription, pts):
                if ch in tradition_to_simple:
                    simple_ch = tradition_to_simple[ch]
                else:
                    simple_ch = ch
                if simple_ch in key_map:
                    label = key_map[simple_ch]
                else:
                    label = key_map["[UNKONWN]"]
                ...
                anno = dict(image_id=image_id, pt=pt, iscrowd=0, category_id=label, id=anno_id, center=[c_x, c_y],
                            bbox=bbox, area=w * h,
                            segmentation=[pt])

                text_annos.append(anno)
        ...
        image_info = dict(file_name=img_name, height=height, width=width, id=image_id, groups=groups)
        images.append(image_info)
        if image_id == 1000:
            val_datas = dict(info=infos, images=images, annotations=annotations, categories=categories)
            with open("val.json", "w") as f:
                json.dump(val_datas, f)
            images = []
            annotations = []

    train_datas = dict(info=infos, images=images, annotations=annotations, categories=categories)

    with open("train.json", "w") as f:
        json.dump(train_datas, f)


convert_front_to_coco(front_path="datas/train/all_raw_anno.json")
